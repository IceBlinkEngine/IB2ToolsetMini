...
namespace IB2ToolsetMini
{
    public class BitmapStringConversion
    {
        ...
        {
            bitmap = new Bitmap(imd.width, imd.height, PixelFormat.Format32bppArgb);
            for (int y = 0; y < bitmap.Height; y++)
            {
                for (int x = 0; x < bitmap.Width; x++)
                {
        ...
        /*public List<ImageData> ReadJsonStringLine(string filename)
        {
            List<ImageData> returnJson = new List<ImageData>();
            using (StreamReader sr = File.OpenText(filename))
            {
                ...
                for (int i = 1; i <= 9999; i++)
                {
                    s = sr.ReadLine();
                    if (s == null) { break; }
                    ImageData newIMD = (ImageData)JsonConvert.DeserializeObject(s, typeof(ImageData));
                    returnJson.Add(newIMD);
                }
                return returnJson;
            }
        }*/        
